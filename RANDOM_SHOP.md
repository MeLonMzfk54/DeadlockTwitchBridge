...
Режим **`full`** — стадия `voting_combined`: тип и тир голосуются **одновременно**. Парсятся **все слова** сообщения:

| Сообщение | Голос |
|-----------|--------|
| `!w 1`, `!w !1`, `w 1`, `!1 !w` | weapon + T1 |
| `!w` / `weapon` | только тип |
| `!1` / `t2` | только тир |
| `weapon` / `w`, `vitality` / `armor` / `v`, `spirit` / `tech` / `s` | тип |
| `1`–`4` или `t1`–`t4` | тир |

Префикс `!` обычно опционален. Если включено **«Требовать !»** — сообщение должно **начинаться** с `!` (`!w 1` ок; второй токен может быть `1` или `!1`).

Один голос на Twitch-пользователя **на каждую ось** (last-vote-wins независимо для типа и тира). Кнопки в `/control` без `userId` — каждый клик отдельно.

Режимы `category` / `tier` — отдельные окна только для одной оси.
...
| Настройка | Смысл |
|-----------|--------|
| После покупки — новый цикл | После `purchased`, провала или скипа шмотки ждёт паузу и стартует снова (**магазин может быть закрыт**) |
...
| Цикл автостарта | `full` (тип+тир сразу) / `category` / `tier` (также для HUD Start) |
| Категория / полный цикл / Тир / Пауза после покупки | Длительности в секундах (`full` использует длительность категории) |
| Категории (чекбоксы) | Какие варианты в голосовании (минимум одна) |
| Мин/макс тир | Диапазон `1–4`; вне диапазона голоса игнорируются |
| Требовать `!` | Сообщение должно начинаться с `!` (`!w` / `!w 1`) |
| Писать в чат при старте | Объявление + инструкция (шаблоны с `{options}` / `{tierOptions}` / `{seconds}` / `{category}`) |
...
Открытие магазина **само по себе не стартует** голосование — только кнопка в HUD, `/control`, или auto-start после покупки, провала или скипа шмотки.

Если включена только одна категория или один тир — соответствующая **ось пропускается** (голосование только по оставшейся, или сразу apply).
